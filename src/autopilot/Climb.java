...
public class Climb implements PhaseController {
    ...
    public Climb(WayPoint[] wp, int i) {
        actions = new ActionXplane();
        lateral_control = new ControlP(0.005f, -1, 1);
        vertical_guidance = new ControlP(-1f, -15, 15);
    ...
    public ActionXplane computeAction(DataXplane data) {
        ...
        final float DTAS = 90; // references
        final float ALTITUDE_MAX = 2000; // termination conditions
        float lat, lon, tas, roll, track, altitude, pitch, gs; // inputs
        float ailerons, elevators; // outputs
        float DROLL, DPITCH, roll_err, pitch_err, spd_err; // errors
